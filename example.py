...
if True:
  ...
  files = example.reconstruct(dtg=dates,leadtime=leadtimes,file_template=file_template)
...
if True:
  ...
  for run in example2.cases[case].runs:
    ...
    files = example2.cases[case].runs[run].reconstruct(leadtime=[0],file_template='(.*)fp',dtg=dates)
    print(' Run:',run)
    [print('  ', f) for f in files]
